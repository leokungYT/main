...
def search_gachaslot_image(bot):
    ...
    while swipe_count <= max_swipes:
        ...
        pt = bot._find_in_screen('img/gachaslot.png', 0.8)
        if pt:
            return pt
        if swipe_count < max_swipes:
            score = bot._get_similarity_score('img/gachaslot.png')
            print(f"[{bot.device_id}] ไม่พบ gachaslot.png (match ขาวดำ {score:.2f}) - เลื่อนหน้าจอครั้งที่ {swipe_count + 1}")
...
class RangerGearBot(threading.Thread):
    ...
    def process_swap_shop(self):
        ...
        def find2(path, th=0.8):
            """หา 2 ทางที่ 0.8: ภาพสีก่อน ไม่ติดค่อยภาพขาวดำ
            ใช้กับทุกรูปใน swap_shop ยกเว้น hero (คงเข้ม 0.95 เท่าเดิม)
            คืนรูปแบบเดียวกับ ImgSearchADB (list ของจุดกึ่งกลาง)"""
            pos = ImgSearchADB(device._screen_color, path, threshold=th)
            if pos and len(pos) > 0:
                return pos
            pt = device._find_in_screen(path, th)
            if pt:
                return [pt]
            return []
        ...
        def check_gachaout_after_click(timeout=3):
            ...
            while time.time() - start_time < timeout:
                try:
                    device.capture_screen()
                    adb_img = device._screen_color
                    gachaout_pos = find2('img/gachaout.png')
                    if gachaout_pos:
        ...
        def priority_check_gachaout(action_name, timeout=8):
            ...
            while time.time() - check_start < timeout:
                try:
                    ...
                    device.capture_screen()
                    img = device._screen_color
                    gachaout_pos = find2('img/gachaout.png') or find2('img/gachaout1.png')
                    if gachaout_pos:
        ...
        def check_and_count_swapgacha1():
            ...
            while time.time() - check_start_time < 3:
                try:
                    device.capture_screen()
                    check_img = device._screen_color
                    swapgacha1_pos = find2('img/swapgacha1.png')
                    if swapgacha1_pos:
        ...
        def check_gacha1(adb_img):
            try:
                gacha1_pos = find2('img/gacha1.png')
                if gacha1_pos:
        ...
        def check_fixbuggacha(adb_img):
            try:
                fixbuggacha_pos = find2('img/fixbuggacha.png')
                if fixbuggacha_pos:
        ...
        while running:
            try:
                ...
                if find2('img/fixrandom1.bmp'):
                    fixrandom2_pos = find2('img/fixrandom2.bmp')
                    if fixrandom2_pos:
                ...
                kaibyswap_pos = find2('img/kaibyswap_shop.png')
                if kaibyswap_pos:
                ...
                clearruby_pos = find2('img/clear-ruby.bmp')
                if clearruby_pos:
                ...
                if not all_in_mode:
                    gachaout_priority_pos = find2('img/gachaout.png') or find2('img/gachaout1.png')
                    ...
                    gachaout1_pos = find2('img/gachaout1.png')
                    if gachaout1_pos:
                ...
                fixunkown_pos = find2('img/fixunkown.png')
                if fixunkown_pos:
                    device.tap(477, 349)
                    time.sleep(1.5)
                    if check_gachaout_after_click(): return "random-Fail"
                ...
                if check_result == "restart": return "restart"
                    ...
                    last_click_position = None
                    continue
                stopgacha7_pos = find2('img/stopgacha7.png')
                if stopgacha7_pos:
                    ...
                    if not found_hero and all_in_mode:
                        ...
                        ok_pos = find2('img/stopgacha6.png')
                        if ok_pos:
                ...
                if stopgachaok_pos:
                    ...
                    continue
                stopgacha4_pos = find2('img/stopgacha4.png')
                if stopgacha4_pos:
                    ...
                    if check_and_count_swapgacha1() == "complete_gacha":
                        ...
                        return "random-Fail"
                    continue
                gachafix_pos = find2('img/gachafix.png')
                if gachafix_pos:
                    if priority_check_gachaout("stopgacha6", 0.1): return "random-Fail"
                    stopgacha6_pos = find2('img/stopgacha6.png')
                    if stopgacha6_pos:
                ...
                if not found_initial_swap_shop:
                    # เครื่องนี้ภาพสีที่ 0.95 หา gacha.png ไม่ติด ทำให้ค้างไม่กดเข้าห้องสุ่ม
                    # ต้องหา 2 ทางเหมือนขั้นเข้าร้านใน shopgacha: ขาวดำ 0.8 ก่อน แล้วค่อยสี 0.8
                    gacha_pt = device._find_in_screen("img/gacha.png", 0.8)
                    if gacha_pt is None:
                        ...
                        while True:
                            try:
                                ...
                                if not found_waitgacha:
                                    if find2('img/waitgacha.png'):
                                        found_waitgacha = True
                                        start_time = time.time()
                                if found_waitgacha:
                                    fixnewgacha_pos = find2('img/fixnewgacha.png')
                                    if fixnewgacha_pos:
                                        device.tap(476, 394)
