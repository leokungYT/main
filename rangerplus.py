...
class RangerPlusBot(multiprocessing.Process):
                ...
                while True:
                    ...
                    if self.exists_in_cache("img/fixid.png", similarity=0.95):
                        ...
                        while time.time() - check_wait_start < 60:
                            ...
                            if err2 in ["fixid", "apple", "fixunkown"]: return "restart"
                            if err2 == "fixbug":
                                self.click("img/fixbuglogin.png")
                                return "restart"
                            if err2 == "icon":
    ...
    def wait_and_click_image(self, img_name, timeout=480):
        ...
        print(f"[{self.device_id}] Waiting for {img_name} (timeout {timeout}s)...")
        start = 0
        while start < timeout:
            try:
                if start > 0 and start % 30 == 0:
                    print(f"[{self.device_id}] Still searching for {img_name}... ({start}/{timeout}s)")
