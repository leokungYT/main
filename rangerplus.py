...
if GUI_AVAILABLE:
    class CollabConfigWindow(ctk.CTkToplevel):
        def __init__(self, parent):
            ...
            # Use a scrollable frame because we have many settings now
            ...
            self.add_switch("🔥 Search ALL Plus", "find_all_rangerplus")
            
            # Character switches for custom_plus
            plus_chars = ["kappa", "anya", "yor", "power", "denji"]
            self.plus_vars = {}
            custom_plus_config = config.get("custom_plus", {})
            for char in plus_chars:
                var = tk.IntVar(value=custom_plus_config.get(char, 0))
                self.plus_vars[char] = var
                chk = ctk.CTkSwitch(self.scroll, text=f"👤 Search {char.capitalize()}", variable=var)
                chk.pack(pady=5, padx=25, anchor="w")
            
            # Thread delay entry
            ctk.CTkLabel(self.scroll, text="System Settings", font=ctk.CTkFont(size=14, weight="bold")).pack(pady=(15, 5))
            self.add_switch("Skip Kaiby Shop (3s)", "kaibyskip")
            self.add_switch("Force Kaiby Check (Proceed)", "kaibycheck")
            self.add_switch("⚡ Minitouch (กดเร็ว)", "minitouch")
            self.add_switch("🧠 จำตำแหน่งปุ่ม (Pos Cache)", "pos_cache")

            delay_frame = ctk.CTkFrame(self.scroll, fg_color="transparent")
            ...
            scan_frame = ctk.CTkFrame(self.scroll, fg_color="transparent")
            ...
            loop_frame = ctk.CTkFrame(self.scroll, fg_color="transparent")
        ...
        def add_switch(self, label, key):
            ...
            self.vars[key] = var
            chk = ctk.CTkSwitch(self.scroll, text=label, variable=var)
            chk.pack(pady=5, padx=25, anchor="w")
        ...
        def save_config(self):
            ...
            # Update custom_plus dictionary
            if "custom_plus" not in config:
                config["custom_plus"] = {}
            for char, var in self.plus_vars.items():
                config["custom_plus"][char] = var.get()
...
class NetworkMonitor:
    ...
    def check_network(self, bot, adb_img):
        ...
        if current_time - self.last_check >= self.check_interval:
            # Note: bot here is the RangerPlusBot instance
...
class RangerPlusBot(multiprocessing.Process):
    ...
    def __init__(self, device_id, args=None):
        ...
        self._screen_raw_png = None  # raw PNG for lazy color decode (fallback path)
    ...
    OTHER_FOLDERS = ["backup", "backup-xml", "login-success", "login-failed", "not-found"]
    ...
    def check_error_images(self, skip_fixcak=False, skip_icon=False):
        ...
        # ===== CRITICAL ERROR CHECKS (ย้ายไฟล์กลับ/สำรองแล้ว restart) =====
    ...
    def inject_file(self, local_xml_path):
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                # 2) แอปที่เพิ่งถูกล้างยังไม่มีโฟลเดอร์ shared_prefs -> cp ล้มเงียบ ๆ สร้างให้ก่อน
                self.adb_shell(
    ...
    def process_kappaplus(self):
        ...
        self.wait_and_click_image("findranger1.png")
        self.wait_and_click_image("findranger2.png")
        ...
        if res:
            print(f"[{self.device_id}] Result: {res}")
            
        self.wait_and_click_image("endfind.png")
        self.wait_and_click_image("resetfind.png")
        self.wait_and_click_image("okfinranger.png")
        sleep(1)
        return res

    def process_anyaplus(self):
        print(f"\n[{self.device_id}] === Starting anyaplus ===")
        self.wait_and_click_image("findranger1.png")
        self.wait_and_click_image("findranger2.png")
        
        if not self.wait_and_click_image("anyafind1.png"):
            return None
        ...
        self.wait_and_click_image("anyafind2.png")
        
        print(f"[{self.device_id}] Swiping down 10 times...")
        for _ in range(10):
            self.swipe(707, 340, 694, 277, 300)
            sleep(0.5)
            
        self.wait_and_click_image("groupbuff.png")
        self.wait_and_click_image("okfinranger.png")
        
        print(f"[{self.device_id}] Swiping (775 440 256 443 300)...")
    ...
    def process_yorplus(self):
        print(f"\n[{self.device_id}] === Starting yorplus ===")
        self.wait_and_click_image("findranger1.png")
        self.wait_and_click_image("findranger2.png")
        
        if not self.wait_and_click_image("yorfind1.png"):
            return None
        ...
        self.wait_and_click_image("yorfind2.png")
    ...
    def process_powerplus(self):
        print(f"\n[{self.device_id}] === Starting powerplus ===")
        self.wait_and_click_image("findranger1.png")
        self.wait_and_click_image("findranger2.png")
        
        if not self.wait_and_click_image("powerfind1.png"):
            return None
        ...
        self.wait_and_click_image("powerfind2.png")
    ...
    def process_denjiplus(self):
        print(f"\n[{self.device_id}] === Starting denjiplus ===")
        self.wait_and_click_image("findranger1.png")
        self.wait_and_click_image("findranger2.png")
        
        if not self.wait_and_click_image("denjifind1.png"):
            return None
        ...
        self.wait_and_click_image("denjifind2.png")
    ...
    def main_login(self, current_filename):
        ...
        while True:
            ...
            # *** SUCCESS -> Run findplus logic ***
            if self.exists_in_cache("img/stoplogin.png"):
                # 1) ต้องผ่าน stoplogin.png ก่อนเท่านั้นถึงจะรัน Plus
                print(f"[{self.device_id}] Login successful (stoplogin confirmed). Starting PLUS process...")
                ...
                # 2) คลิกปุ่มเข้าหน้า Ranger Plus
                if not self.wait_and_click_image("findrangerplus1.png", timeout=30):
                    print(f"[{self.device_id}] [ERR] findrangerplus1.png not found, cannot continue Plus process")
                    self.clear_and_restart()
                    return "failed"
                
                # 3) รอเข้าหน้าสแกน (หา findranger2 เพื่อยืนยันว่าเข้าหน้าถูกแล้ว)
                # (จะถูกเรียกในแต่ละ process_...plus ด้านล่าง)
                
                all_found = []
                
                # Check config to see which plus processes to run
                find_all_plus = config.get("find_all_rangerplus", 1)
                custom_plus = config.get("custom_plus", {})
                
                if find_all_plus or custom_plus.get("kappa", 0):
                    kappa_res = self.process_kappaplus()
                    if kappa_res: all_found.append(kappa_res)
                    
                if find_all_plus or custom_plus.get("anya", 0):
                    anya_res = self.process_anyaplus()
                    if anya_res: all_found.append(anya_res)
                    
                if find_all_plus or custom_plus.get("yor", 0):
                    yor_res = self.process_yorplus()
                    if yor_res: all_found.append(yor_res)
                    
                if find_all_plus or custom_plus.get("power", 0):
                    power_res = self.process_powerplus()
                    if power_res: all_found.append(power_res)
                    
                if find_all_plus or custom_plus.get("denji", 0):
                    denji_res = self.process_denjiplus()
                    if denji_res: all_found.append(denji_res)
                    
                print(f"[{self.device_id}] FINISHED PLUS. Found: {all_found}")
                
                # Combine results and backup
                filename = self.current_original_filename or "unknown.xml"
                source_path = "/data/data/com.linecorp.LGRGS/shared_prefs/_LINE_COCOS_PREF_KEY.xml"
                
                found_names = "-".join(sorted(set(all_found))) if all_found else "unknown"
                
                if all_found:
                    msg = f"[{self.device_id}] 🏆 Success! Found: {found_names}"
                    if GUI_INSTANCE:
